...
class RouteServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        Route::macro('webPermissionResource', function ($model, $controller, $options = []) {
            $guardName = $options['guard_name'] ?? 'admin';
            $methods = ['index', 'show', 'create', 'store', 'edit', 'update', 'destroy'];
            if (isset($options['except'])) {
                foreach ($options['except'] as $except) {
                    unset($methods[array_search($except, $methods)]);
            ...
            foreach ($methods as $method) {
                ...
                $url = match ($method) {
                    'index', 'store', 'create' => $model,
                    'update', 'edit', 'destroy', 'show' => $model . "/{" . str_replace('-', '_', Str::singular($model)) . '}',
                };
                Route::name($model . '.' . $method)->{$realMethod}($url, $controller . '@' . $method)
                    ->middleware('permission:' . $action . '_' . ($options['permission_name'] ?? Str::singular($model)) . ',' . $guardName);
            }
        ...
        \Illuminate\Routing\Route::macro('hasPermission', function ($permission, $guardName = 'admin-api') {
