...
@section('content')
    ...
    <x-card>
        <x-slot name="cardTitle">لیست منو های ({{ $menu->title }})</x-slot>
        <x-slot name="cardOptions"><x-card-options /></x-slot>
        <x-slot name="cardBody">
            @include('components.errors')
            <form id="myForm" action="{{ route('admin.menu.sort', [$menu_items[0]->group_id, $menu]) }}" method="POST">
                @csrf
                @method('PATCH')
                                    ...
                                    <td class="">
                                        ...
                                        @else
                                            <a class="text-info" data-original-title="مشاهده"
                                                href="{{ route('admin.menu.grandchildIndex', [$group, $menu_item]) }}">{{ $menu_item->title }}</a>
                                        @endif
                                    </td>
                                    <td>
                                        @if ($count == 0)
                                        ...
                                        @else
                                            <a class="text-info" data-original-title="مشاهده"
                                                href="{{ route('admin.menu.grandchildIndex', [$group, $menu_item]) }}">{{ $count }}</a>
                                        @endif
                                    </td>
                                    <td>@include('core::includes.status', ['status' => $menu_item->status])</td>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="parent_id" class="form-control select2">
                            <option value="{{ $menu->id }}" selected>- انتخاب کنید -</option>
                            @foreach ($menu_items as $menu_item)
