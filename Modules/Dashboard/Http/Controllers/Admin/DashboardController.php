...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $salesAmountByMonth = $this->reportService
            ->salesAmountByDate(now()->startOfMonth(), Carbon::now()->endOfMonth());
        $ordersCount = $order->whereNull('parent_id')->count();
        $todayOrdersCount = $order->whereNull('parent_id')->whereBetween('created_at',
            [Carbon::createFromTimestamp(verta()->startDay()->getTimestamp()),
                Carbon::now()
            ])
            ->count();
        $lastOrders = Order::withCommonRelations()->whereNull('parent_id')->latest('id')->take(7)->get()->append('total_amount');
