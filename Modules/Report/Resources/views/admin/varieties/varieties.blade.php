...
@section('content')
    <div class="page-header">
        <x-breadcrumb :items="[['title' => 'گزارش تنوع ها']]" />
    </div>
                ...
                <x-slot name="tableTd">
                    @forelse($varieties as $variety)
                        <tr>
                            ...
                            <td>{{ $variety->color?->name ?? '-' }}</td>
                            <td>{{ $variety->attributes?->first()->pivot->value ?? '-' }}</td>
                            <td>{{ number_format($variety->total_sales['sales_count']) }}</td>
                            <td>{{ number_format($variety->total_sales['sales_amount']) }}</td>
                        </tr>
                    @empty
